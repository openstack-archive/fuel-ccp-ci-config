...
# CONFIGURATION:
######################################################
FUEL_DEVOPS_ENV_NAME="ccp-"${COMPONENT}"-deployment-env"
...
${SSH_COMMAND} cd ccp-repos && git fetch "${ZUUL_URL}"/"${ZUUL_PROJECT}" "${ZUUL_REF}" && git checkout FETCH_HEAD && git checkout FETCH_HEAD
