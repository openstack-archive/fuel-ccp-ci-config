...
# Download CCP repos sources, patch it with change for component,
# build everything, deploy and then runs simple, built-in OpenStack tests.
...
# CONFIGURATION:
######################################################
FUEL_DEVOPS_ENV_NAME="ccp-"${COMPONENT}"-${VERSION}-deployment-env"
...
${SSH_COMMAND} rm -rf ccp-repos/"${ZUUL_PROJECT}"
