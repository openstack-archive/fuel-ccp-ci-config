...
for f in ${IMG}; do
    sshpass  -p vagrant "ssh vagrant@$MASTER_IP docker tag $f ${DOCKER_REGISTRY}/${IMAGES_NAMESPACE}/$f:<tag_nr-fail-or-ok>"
    sshpass  -p vagrant "ssh vagrant@$MASTER_IP docker push ${DOCKER_REGISTRY}/${IMAGES_NAMESPACE}/$f:<tag_nr-fail-or-ok>"
done
